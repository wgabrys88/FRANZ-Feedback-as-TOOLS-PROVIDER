...
Supports two mark modes controlled by config:
  MARKS_CLASSIC -- numbered circles, diamonds, arrows per action
  MARKS_CURSOR  -- cursor icon with normalized coordinates at current
                   and previous positions

Reads run_dir from the incoming JSON request to locate sandbox_canvas.bmp
and sandbox_state.json within the per-execution run directory.
"""
...
MARK_FILL: Final[Color] = (255, 0, 0, 140)
MARK_OUTLINE: Final[Color] = (255, 255, 255, 180)
MARK_TEXT: Final[Color] = (255, 255, 255, 255)
TRAIL_COLOR: Final[Color] = (255, 0, 0, 50)
...
class Canvas:
    ...
    def arrow(self, x1: int, y1: int, x2: int, y2: int, c: Color, t: int) -> None:
        ...
        ha, ln = math.radians(25.0), float(_ms(22))
        self.fill_polygon([
            (x2, y2),
...
def _render_digit(cv: Canvas, cx: int, cy: int, d: int, fill: Color, outline: Color, scale: int) -> None:
    gw = 5 * scale
    ox, oy = cx - gw // 2, cy - (7 * scale) // 2
    g = _DIGITS[d]
    for ddy in (-1, 0, 1):
        for ddx in (-1, 0, 1):
            if ddx == 0 and ddy == 0:
                continue
            for ri, row_bits in enumerate(g):
                for ci in range(5):
                    if row_bits & (1 << (4 - ci)):
                        for sy in range(scale):
                            for sx in range(scale):
...
def _apply_marks_classic(buf: bytearray, w: int, h: int, actions: list[str]) -> None:
    cv = Canvas(buf, w, h)
    ...
    for name, args, kw in entries:
        if name in ("left_click", "right_click", "double_left_click"):
            ...
            if x0 is not None and y0 is not None:
                positions.append((_norm(x0, w), _norm(y0, h)))
            else:
                positions.append(None)
        elif name == "drag":
            x1, y1 = _arg_int(args, kw, 0, "x1"), _arg_int(args, kw, 1, "y1")
            if x1 is not None and y1 is not None:
                positions.append((_norm(x1, w), _norm(y1, h)))
            else:
                positions.append(None)
        else:
            positions.append(None)
    ...
    for name, args, kw in entries:
        match name:
            case "left_click":
                ...
                cv.circle(x, y, _ms(24), MARK_OUTLINE, True, ot)
                cv.circle(x, y, _ms(20), MARK_FILL, True, ot)
            ...
            case "right_click":
                ...
                _render_number(cv, x, y, n, MARK_TEXT, BLACK, _ms(2))
                lcp_x, lcp_y = x, y; n += 1
            case "double_left_click":
                ...
                x, y = _norm(x0, w), _norm(y0, h)
                rt = _ms(3)
                cv.circle(x, y, _ms(28), MARK_OUTLINE, False, rt)
                cv.circle(x, y, _ms(18), MARK_OUTLINE, False, rt)
                cv.circle(x, y, _ms(14), MARK_FILL, True, ot)
                _render_number(cv, x, y, n, MARK_TEXT, BLACK, _ms(2))
                lcp_x, lcp_y = x, y; n += 1
            case "drag":
                x1, y1 = _arg_int(args, kw, 0, "x1"), _arg_int(args, kw, 1, "y1")
                ...
                px1, py1 = _norm(x1, w), _norm(y1, h)  # type: ignore[arg-type]
                px2, py2 = _norm(x2, w), _norm(y2, h)  # type: ignore[arg-type]
                cv.arrow(px1, py1, px2, py2, MARK_FILL, _ms(4))
                cv.circle(px1, py1, _ms(14), MARK_OUTLINE, True, ot)
                cv.circle(px1, py1, _ms(10), MARK_FILL, True, ot)
                _render_number(cv, px1, py1, n, MARK_TEXT, BLACK, _ms(2))
                cv.circle(px2, py2, _ms(14), MARK_OUTLINE, False, _ms(3))
                cv.circle(px2, py2, _ms(10), MARK_FILL, False, _ms(2))
            ...
            case "type":
                ...
                cv.line(ux, uy, ux + tw, uy, MARK_FILL, _ms(3))
                cv.line(ux, uy + _ms(2), ux + tw, uy + _ms(2), MARK_OUTLINE, _ms(1))
                _render_number(cv, ux + tw + _ms(12), uy - _ms(4), n, MARK_TEXT, BLACK, _ms(2))
                n += 1
...
def _apply_marks_cursor(
    ...
    for line in actions:
        ...
        if parsed is None:
            continue
        name = parsed[0]
        if name in ("timestamp", "screenshot"):
            stamp = datetime.now().strftime("CAPTURED %Y-%m-%d %H:%M:%S")
            sc = _ms(2)
            cw = 6 * sc
            ch = 7 * sc
...
def capture(
    ...
    if marks and actions:
        marks_classic = bool(getattr(franz_config, "MARKS_CLASSIC", True))
        marks_cursor = bool(getattr(franz_config, "MARKS_CURSOR", False))
        if marks_classic:
            _apply_marks_classic(rgba, sw, sh, actions)
        if marks_cursor and state_path is not None:
